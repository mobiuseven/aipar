...
int dilation(unsigned char **brightness, unsigned char hdr[], char fname[], int width, int height){
  FILE *fp ;
...
int main(void){
  ...
  char imgOUT[] = "image_1_dilation.bmp" ;
  unsigned char hdr[54] ;
  unsigned char **brightness ;
  image bmp ;
  FILE *fp ;
  int i = 0 ;
  int imgRead, mkDilation ;
  ...
  //apply median filter
  mkDilation = dilation(brightness, hdr, imgOUT, bmp.width, bmp.height) ;
  
  free(brightness) ;
  
  printf("Finished dilation\n") ;
